...
### AI 聊天核心修復（2026-09-21 更新）

- **真流式輸出**：聊天引擎改為大模型流式接口（`stream: true`），邊生成邊推送，告別「等待後一次性輸出」；保留供應商自動降級（輸出前失敗自動切換，輸出後失敗直接報錯避免重複），兼容不支持 `stream_options` 的供應商
- **trace_id 全鏈路追蹤**：每次對話生成唯一追蹤標識（時間戳+隨機數），貫穿服務端日誌（工具調用/降級/完成），並作為首個 SSE 事件返回前端；聊天界面底部展示 `trace: xxx`，點擊複製後可在日誌中精確檢索該次對話全部記錄
- **本地新聞 401 修復**：`local_market_data` 的本地新聞查詢補上 `X-API-Key` 認證頭（與統一後端客戶端一致），修復後端開啟認證後無法引用已入庫新聞的問題
- **FTShare 工具名更新**：FTShare 服務端升級後全部工具加 `ft_v1_` 前綴（如個股 K 線 `ft_v1_stock_candlesticks`），工具描述已同步更新，舊名 `get_kline` 已失效
- **工具調用去重**：流式輸出時過濾內聯工具標記，工具狀態統一在內容上方展示，修復「文字上下重複顯示工具調用」
- **ChatGPT 風格細節**：用戶消息右對齊深灰氣泡（右下小圓角）、AI 消息無氣泡全寬排版、白色方形打字光標、大圓角輸入框內嵌圓形發送按鈕（懸停放大/按下縮小）；響應式適配（氣泡寬度移動端 85%/桌面 70%、字號與間距分級）

### 跨平台運行

本項目具備跨平台運行特質，支持 **Windows / macOS / Linux** 三大平台：

- **Windows**：每個服務目錄提供一鍵啟動腳本 `start.ps1`（PowerShell）
- **macOS / Linux**：提供 Bash 啟動腳本，亦可通過 systemd（`trading-java` / `trading-agent` / `trading-workstation-frontend`）託管為系統服務
- **Docker**：提供 `docker-compose.yml` 與 `deploy-docker.sh`，一鍵容器化部署（跨平台一致環境）
- 前端（Next.js）、後端（Java）、AI 服務（Python）均為跨平台技術棧，無平台綁定依賴
