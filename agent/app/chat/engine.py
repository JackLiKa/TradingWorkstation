...
MAX_TOOL_ROUNDS = 15  # 安全護欄：限制最大工具調用輪數，防止成本攻擊和 DoS
...
class ChatEngine:
        ...
        for round_idx in range(MAX_TOOL_ROUNDS):
            ...
            if response.get("tool_calls"):
                ...
                for tool_call in response["tool_calls"]:
                    ...
                    # 安全護欄：截斷超長工具結果，防止撐爆上下文窗口
                    _content = tool_result.content
                    if len(_content) > 32000:
                        _content = _content[:32000] + "\n\n[結果已截斷：原始長度 " + str(len(tool_result.content)) + " 字符]"
    ...
    async def chat_stream(
        ...
        Args:
            trace_id: 本次對話的唯一追蹤標識，貫穿日誌記錄，便於問題排查。
        """
        _trace = f"trace_id={trace_id} " if trace_id else ""
        self._ensure_tools()
        ...
        for round_idx in range(MAX_TOOL_ROUNDS):
            ...
            try:
                response = None
                async for kind, payload in self._stream_llm_with_fallback(
                    tool_provider, openai_messages, tools, self._get_fallback_chain(tool_provider)
                ):
                    if kind == "content":
                        ...
                        response = payload
                if response is None:
                    raise RuntimeError("LLM 未返回有效響應")
            ...
            if response.get("tool_calls"):
                ...
                for tool_call in response["tool_calls"]:
                    ...
                    logger.info(f"[聊天工具] {_trace}調用 {tool_name}({args})")
                    yield json.dumps({
                        "type": "tool_start",
                        "tool": tool_name,
                        "arguments": args,
                    ...
                    logger.info(f"[聊天工具] {_trace}{tool_name} 完成: success={tool_result.success}, citations={len(tool_result.citations)}")
                    yield json.dumps({
                        "type": "tool_end",
                        "tool": tool_name,
                        "success": tool_result.success,
                ...
                # 內容已在流式調用中逐塊推送給前端，此處直接發送完成事件
                logger.info(f"[聊天] {_trace}對話完成 (provider={tool_provider}, tokens={response.get('tokens', 0)}, tools={len(all_tool_logs)})")
                yield json.dumps({
                    "type": "done",
                    "provider": tool_provider,
    ...
    async def _stream_llm_with_fallback(
        ...
        """帶降級的流式 LLM 調用 — 逐塊 yield ("content", text)，最後 yield ("response", dict)。

        若某供應商在輸出內容前失敗（429/401/403 等），自動降級到下一個供應商；
        已開始輸出內容後失敗則直接拋錯（避免內容重複輸出）。
        """
        last_error: Exception | None = None
        for p in fallback_chain:
            started = False
            try:
                async for item in self._stream_llm_with_tools(p, messages, tools):
                    kind, payload = item
                    if kind == "content":
                        started = True
                    yield item
                ...
                if started:
                    raise
                logger.warning(f"[聊天] 供應商 {p} 流式調用失敗: {e}，降級到下一個")
                continue
        raise RuntimeError(f"所有工具調用供應商均不可用: {last_error}")
    ...
    async def _stream_llm_with_tools(
        ...
        """流式調用 LLM（帶工具定義）— yield ("content", chunk)，最後 yield ("response", dict)。"""
        ...
        async def _do_stream(use_stream_options: bool):
            ...
            if tool_calls_acc:
                ...
                yield ("response", {"content": "", "tool_calls": tool_calls, "message": message, "tokens": tokens})
            else:
                if not content:
                    raise RuntimeError(f"{info.display_name} 返回空字符串")
                message = {"role": "assistant", "content": content}
                yield ("response", {"content": content, "tool_calls": None, "message": message, "tokens": tokens})

        try:
            async for item in _do_stream(True):
                yield item
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 400:
                # 部分供應商不支持 stream_options，去掉後重試
                async for item in _do_stream(False):
                    yield item
            else:
                raise
